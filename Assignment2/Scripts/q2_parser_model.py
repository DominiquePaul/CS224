...
class ParserModel(Model):
    ...
    def add_embedding(self):
        ...
        """
        ### YOUR CODE HERE
        embed1 = tf.Variable(self.pretrained_embeddings)
        embed2 = tf.nn.embedding_lookup(self.pretrained_embeddings, self.input_placeholder)
        embeddings = tf.reshape(embed2, [-1, self.config.n_features * self.config.embed_size])
        ### END YOUR CODE
        return embeddings
    ...
    def add_prediction_op(self):
        ...
        ### YOUR CODE HERE
        xavier_initiailiser = xavier_weight_init()
        W = xavier_initiailiser(shape=(self.config.n_features*self.config.embed_size, self.config.hidden_size))
        b1 = tf.Variable(tf.zeros(self.config.hidden_size,))
        U = xavier_initiailiser(shape=(self.config.hidden_size, self.config.n_classes))
