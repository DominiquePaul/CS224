...
class SoftmaxModel(Model):
    ...
    def create_feed_dict(self, inputs_batch, labels_batch=None):
        ...
        if self.labels_placeholder is not None:
    ...
    def add_prediction_op(self):
        ...
        """
        ### YOUR CODE HERE
        weight = tf.Variable(tf.random_uniform((self.config.n_features, self.config.n_classes),0,0))
        bias = tf.Variable(tf.zeros((self.config.n_classes)))
        y = tf.matmul(self.input_placeholder, weight) + bias
        pred = softmax(y)
